...
Original file is located at
    https://colab.research.google.com/drive/1QxbC0Mmy97q7XoZkdsEnBz0ibO5O0pvD
"""
...
def windowed_dataset(series, batch_size, n_past=24, n_future=24, shift=1):
    # YOUR CODE HERE
    x = tf.data.Dataset.from_tensor_slices(series)
    x = x.window(size = n_past + n_future, shift = shift, drop_remainder = True)
    x = x.flat_map(lambda w: w.batch(n_past + n_future))
    x = x.map(lambda w: (w[:-n_past], w[-n_past:, :1]))
    return x.batch(batch_size).prefetch(1)
